...
def set_drophead_all(model, rate):
    for name, module in model.named_modules():
        if hasattr(module, "set_drophead_rate"):
            module.set_drophead_rate(rate)
...
class Trainer:
    ...
    def run(
        ...
        for iepoch in range(start_epoch, trainer_options.max_epoch + 1):
            ...
            def drophead_scheduler(epoch, num_epochs, p_start=0.0, p_end=0.0):
                #mid_epoch = 26 #num_epochs // 2
                mid_epoch = 15 #num_epochs // 2
                if epoch <= mid_epoch:
                    # Decrease p from p_start to 0
                    p = p_start * (1 - epoch / mid_epoch)
                else:
                    # Increase p from 0 to p_end
                    p = p_end * ((epoch - mid_epoch) / (num_epochs - mid_epoch))
                return p
            
            p_drophead = trainer_options.p_drophead
        ...
        for iiter, (utt_id, batch) in enumerate(
            ...
            with reporter.measure_time("backward_time"):
                if scaler is not None:
                    ...
                    #print(model.grad)
                    #print("grad:", {
                        #name: param.grad.norm().item() if param.grad is not None else None
            ...
            if iiter % accum_grad == 0:
                ...
                #for param_group in optimizer.param_groups:
                    #for param in param_group["params"]:
                        #print(param)
                        #print("Theta is in optimizer with LR:", param_group["lr"])
